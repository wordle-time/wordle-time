...
describe(feedback_group, () => {
  ...
  it(correct_letter_test, () => {
    ...
    cy.screenshot(
      getScreenshotPath('F002', feedback_group, correct_letter_test),
      {
        overwrite: true,
      }
    );
  ...
  const wrong_spot_test = 'should show wrong_spot';
  it(wrong_spot_test, () => {
    // if wrong char is entered, guess button should be disabled && input field should be empty
    cy.get('[data-cy="letter-0"]').clear();
  ...
  it(wrong_letter_test, () => {
    ...
    cy.screenshot(
      getScreenshotPath('F002', feedback_group, wrong_letter_test),
      {
        overwrite: true,
      }
    );
